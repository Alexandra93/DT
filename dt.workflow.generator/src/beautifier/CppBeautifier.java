...
public class CppBeautifier implements PostProcessor {
	...
	public void beforeWriteAndClose(final FileHandle fileHandle) {
		if (fileHandle.getTargetFile().toString().endsWith(".cs")
				|| fileHandle.getTargetFile().toString().endsWith(".py")) {
			...
			try {
				...
				IASTTranslationUnit ast = createTranslationUnit(
						settings.getMap(), source);
				IDocument document = new Document(source);
				codeFormatter.format(source, ast).apply(document);
				fileHandle.setBuffer(document.get().replaceAll("\r", ""));
			} catch (Exception e) {
	...
	private DefaultCodeFormatterOptions getCodeFormatterSettings() {
		if (formatterOptions == null) {
			formatterOptions = DefaultCodeFormatterOptions.getDefaultSettings();
			formatterOptions.line_separator = System
	...
	/**
	 * Return a Java Properties instance representing the options that are in
	 * the specified configuration file. In order to use this, simply export a
